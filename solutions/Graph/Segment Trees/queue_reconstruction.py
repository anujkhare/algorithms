...
## Time complexity:: insert:, update:, query:
## Space complexity:: O()
## Notes::
## Bookmarked: Yes
...
class Solution:
    ...
    def order(self, heights, infronts):
        ...
        hpos = sorted([
            (height, infront)
            for height, infront in zip(heights, infronts)
        ],
            key=lambda x: x[0])

        result = [None] * l
        stree = SegmentTreeR([1] * l)

        for height, infront in hpos:
            pos = stree.find_cumsum_ind(infront + 1)

            if pos is None: raise ValueError
            result[pos] = height
            stree.update(pos, 0)
